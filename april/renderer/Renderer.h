...
class RenderQueue {
 ...
 public:
  ...
  void clear();
  /**Realloc command queues and reserve with given size. Note: this clears any
   * existing commands.*/
...
Whenever possible prefer to use `TrianglesCommand` objects since the renderer
will automatically batch them.
 */
class Renderer {
 public:
  ...
  /**The rendercommands which can be batched will be saved into a list, this is
   * the reserved size of this list.*/
  static const int BATCH_TRIAGCOMMAND_RESERVED_SIZE = 64;
  /**Reserved for material id, which means that the command could not be
   * batched.*/
  ...
  /** Adds a `RenderComamnd` into the renderer specifying a particular render
   * queue ID */
  ...
  /**
   Set render targets. If not set, will use default render targets. It will
   effect all commands.
   @flags Flags to indicate which attachment to be replaced.
   @colorAttachment The value to replace color attachment, only one color
   attachment supported now.
   @depthAttachment The value to repalce depth attachment.
   @stencilAttachment The value to replace stencil attachment. Depth attachment
   and stencil attachment can be the same value.
   */
  void setRenderTarget(RenderTargetFlag flags, Texture2D* colorAttachment,
  ...
  */
  void clear(ClearFlag flags, const Color4F& color, float depth,
   ...
   * @param value true means enable writing into the depth buffer, otherwise
   * false.
   */
   ...
   * @readMask Specifies a mask that is ANDed with both the reference value and
   * the stored stencil value when the test is done.
   */
  void setStencilCompareFunction(backend::CompareFunction func,
                                 unsigned int ref, unsigned int readMask);

  /**
   * Set front and back stencil test actions.
   * @param stencilFailureOp Specifies the action to take when the stencil test
   * fails.
   * @param depthFailureOp Specifies the stencil action when the stencil test
   * passes, but the depth test fails.
   * @param stencilDepthPassOp Specifies the stencil action when both the
  ...
  /// Get the stencil action when the stencil test passes, but the depth test
  /// fails.
  backend::StencilOperation getStencilPassDepthFailureOperation() const;

  /// Get the stencil action when both the stencil test and the depth test pass,
  /// or when the stencil test passes and either there is no depth buffer or
   ...
   * @see `setStencilCompareFunction(backend::CompareFunction func, unsigned int
   * ref, unsigned int readMask)`
   */
   ...
   * @param mode Controls if primitives are culled when front facing, back
   * facing, or not culled at all.
   */
 ...
 protected:
   ...
   * When queued vertex or index count exceed the limited value, a new vertex or
   * index buffer will be created.
   */
  class TriangleCommandBufferManager {
   public:
     ...
     * That means when get vertex buffer or index buffer, the earliest created
     * buffer object in the cache will be returned.
     */
    void putbackAllBuffers();

    /**
     * Buffer will be created If next buffer unavailable in the cache, otherwise
     * set the buffer index in order to get the next available buffer.
     */
  ...
  /**
   * Building a programmable pipeline involves an expensive evaluation of GPU
   * state. So a new render pipeline object will be created only if it hasn't
   * been created before.
  ...
  void setRenderPipeline(const PipelineDescriptor&,
                         const backend::RenderPassDescriptor&);
  ...
  Winding _winding =
      Winding::COUNTER_CLOCK_WISE;  // default front face is CCW in GL
