...
enum class BufferType : uint32_t { VERTEX, INDEX };

enum class ShaderStage : uint32_t { VERTEX, FRAGMENT, VERTEX_AND_FRAGMENT };
...
enum class TextureUsage : uint32_t { READ, WRITE, RENDER_TARGET };

enum class IndexFormat : uint32_t { U_SHORT, U_INT };
...
enum class TextureType : uint32_t { TEXTURE_2D, TEXTURE_CUBE };
...
enum class BlendOperation : uint32_t { ADD, SUBTRACT, RESERVE_SUBTRACT };
...
struct SamplerDescriptor {
  ...
  SamplerDescriptor(SamplerFilter _magFilter, SamplerFilter _minFilter,
...
enum class Winding : uint32_t { CLOCK_WISE, COUNTER_CLOCK_WISE };

struct UniformInfo {
  int count = 0;
  int location = -1;

  // in opengl, type means uniform data type, i.e. GL_FLOAT_VEC2, while in metal
  // type means data basic type, i.e. float
  unsigned int type = 0;
  bool isArray = false;
  unsigned int size = 0;
...
struct UniformLocation {
   ...
   * in opengl, location[0] represent the location, and location[1] represent
   * location offset in uniform buffer.
   */
  int location[2] = {-1, -1};
...
enum class ProgramType : size_t {
  POSITION_COLOR_LENGTH_TEXTURE,        // positionColorLengthTexture_vert,
                                        // positionColorLengthTexture_frag
  POSITION_COLOR_TEXTURE_AS_POINTSIZE,  // positionColorTextureAsPointsize_vert,
                                        // positionColor_frag
  ...
  POSITION_TEXTURE_COLOR_ALPHA_TEST,  // positionTextureColor_vert,
  ...
  LABLE_DISTANCEFIELD_GLOW,  // positionTextureColor_vert,
                             // labelDistanceFieldGlow_frag
  LABEL_DISTANCE_NORMAL,     // positionTextureColor_vert,
                             // label_distanceNormal_frag
  ...
  SKINPOSITION_TEXTURE_3D,         // CC3D_skinPositionTexture_vert,
                                   // CC3D_colorTexture_frag
  SKINPOSITION_NORMAL_TEXTURE_3D,  // CC3D_skinPositionNormalTexture_vert,
                                   // CC3D_colorNormalTexture_frag
  POSITION_NORMAL_TEXTURE_3D,      // CC3D_positionNormalTexture_vert,
                                   // CC3D_colorNormalTexture_frag
  ...
  POSITION_BUMPEDNORMAL_TEXTURE_3D,      // CC3D_positionNormalTexture_vert,
                                         // CC3D_colorNormalTexture_frag
  SKINPOSITION_BUMPEDNORMAL_TEXTURE_3D,  // CC3D_skinPositionNormalTexture_vert,
                                         // CC3D_colorNormalTexture_frag
