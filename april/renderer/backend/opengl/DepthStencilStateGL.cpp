...
void DepthStencilStateGL::apply(unsigned int stencilReferenceValueFront,
  ...
  glDepthFunc(
      UtilsGL::toGLComareFunction(_depthStencilInfo.depthCompareFunction));
  ...
  if (_depthStencilInfo.stencilTestEnabled) {
    if (_isBackFrontStencilEqual) {
      ...
      glStencilFuncSeparate(
          GL_BACK,
          UtilsGL::toGLComareFunction(
              _depthStencilInfo.backFaceStencil.stencilCompareFunction),
      ...
      glStencilMaskSeparate(GL_BACK,
                            _depthStencilInfo.backFaceStencil.writeMask);
