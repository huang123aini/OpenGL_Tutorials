...
CommandBufferGL::~CommandBufferGL() {
  ...
  SAFE_RELEASE_NULL(_renderPipeline);
...
void CommandBufferGL::applyRenderPassDescriptor(
    const RenderPassDescriptor& descirptor) {
  bool useColorAttachmentExternal =
      descirptor.needColorAttachment && descirptor.colorAttachmentsTexture[0];
  bool useDepthAttachmentExternal =
      descirptor.depthTestEnabled && descirptor.depthAttachmentTexture;
  bool useStencilAttachmentExternal =
      descirptor.stencilTestEnabled && descirptor.stencilAttachmentTexture;
  bool useGeneratedFBO = false;
  if (useColorAttachmentExternal || useDepthAttachmentExternal ||
  ...
  if (descirptor.needColorAttachment) {
    ...
    for (const auto& texture : descirptor.colorAttachmentsTexture) {
      if (texture) {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i,
                               GL_TEXTURE_2D, getHandler(texture), 0);
      }
      CHECK_GL_ERROR_DEBUG();
      ++i;
    }

    if (useGeneratedFBO) _generatedFBOBindColor = true;
    ...
    // If not draw buffer is needed, should invoke this line explicitly, or it
    // will cause GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER and
    // GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER error.
...
void CommandBufferGL::setRenderPipeline(RenderPipeline* renderPipeline) {
  ...
  if (renderPipeline == nullptr) return;
...
void CommandBufferGL::setViewport(int x, int y, unsigned int w,
                                  unsigned int h) {
  glViewport(x, y, w, h);
  _viewPort.x = x;
  _viewPort.y = y;
...
void CommandBufferGL::setCullMode(CullMode mode) { _cullMode = mode; }
...
void CommandBufferGL::setVertexBuffer(Buffer* buffer) {
  assert(buffer != nullptr);
  if (buffer == nullptr || _vertexBuffer == buffer) return;
...
void CommandBufferGL::drawArrays(PrimitiveType primitiveType, std::size_t start,
                                 std::size_t count) {
  prepareDrawing();
...
void CommandBufferGL::drawElements(PrimitiveType primitiveType,
                                   IndexFormat indexType, std::size_t count,
                                   std::size_t offset) {
  prepareDrawing();
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _indexBuffer->getHandler());
  glDrawElements(UtilsGL::toGLPrimitiveType(primitiveType), count,
                 UtilsGL::toGLIndexType(indexType), (GLvoid*)offset);
  CHECK_GL_ERROR_DEBUG();
  cleanResources();
}
...
void CommandBufferGL::setDepthStencilState(
...
void CommandBufferGL::bindVertexBuffer(ProgramGL* program) const {
  ...
  if (!vertexLayout->isValid()) return;
  ...
  for (const auto& attributeInfo : attributes) {
    ...
    glVertexAttribPointer(
        attribute.index_, UtilsGL::getGLAttributeSize(attribute.format_),
        UtilsGL::toGLAttributeType(attribute.format_),
        attribute.need_to_be_normallized_, vertexLayout->getStride(),
...
void CommandBufferGL::setUniforms(ProgramGL* program) const {
  if (_programState) {
    ...
    for (auto& iter : uniformInfos) {
      ...
      if (uniformInfo.size <= 0) continue;

      int elementCount = uniformInfo.count;
      setUniform(uniformInfo.isArray, uniformInfo.location, elementCount,
                 uniformInfo.type, (void*)(buffer + uniformInfo.bufferOffset));
    }
...
void CommandBufferGL::setUniform(bool isArray, GLuint location,
                                 unsigned int size, GLenum uniformType,
                                 void* data) const {
  GLsizei count = size;
  switch (uniformType) {
    case GL_INT:
    ...
    case GL_BOOL_VEC3:
      if (isArray)
        ...
        glUniform3i(location, DEF_TO_INT(data, 0), DEF_TO_INT(data, 1),
                    DEF_TO_INT(data, 2));
      break;
    case GL_INT_VEC4:
    case GL_BOOL_VEC4:
      if (isArray)
        ...
        glUniform4i(location, DEF_TO_INT(data, 0), DEF_TO_INT(data, 1),
                    DEF_TO_INT(data, 2), DEF_TO_INT(data, 4));
      break;
    case GL_FLOAT:
      if (isArray)
    ...
    case GL_FLOAT_VEC3:
      if (isArray)
        ...
        glUniform3f(location, DEF_TO_FLOAT(data, 0), DEF_TO_FLOAT(data, 1),
                    DEF_TO_FLOAT(data, 2));
      break;
    case GL_FLOAT_VEC4:
      if (isArray)
        ...
        glUniform4f(location, DEF_TO_FLOAT(data, 0), DEF_TO_FLOAT(data, 1),
                    DEF_TO_FLOAT(data, 2), DEF_TO_FLOAT(data, 3));
      break;
    case GL_FLOAT_MAT2:
...
void CommandBufferGL::cleanResources() {
  SAFE_RELEASE_NULL(_indexBuffer);
  SAFE_RELEASE_NULL(_programState);
  SAFE_RELEASE_NULL(_vertexBuffer);
}
...
void CommandBufferGL::setScissorRect(bool isEnabled, float x, float y,
...
void CommandBufferGL::captureScreen(
    std::function<void(const unsigned char*, int, int)> callback) {
  int bufferSize = _viewPort.w * _viewPort.h * 4;
  std::shared_ptr<GLubyte> buffer(new GLubyte[bufferSize],
                                  [](GLubyte* p) { SAFE_DELETE_ARRAY(p); });
  ...
  glReadPixels(0, 0, _viewPort.w, _viewPort.h, GL_RGBA, GL_UNSIGNED_BYTE,
               buffer.get());

  std::shared_ptr<GLubyte> flippedBuffer(
      new GLubyte[bufferSize], [](GLubyte* p) { SAFE_DELETE_ARRAY(p); });
  ...
  for (int row = 0; row < _viewPort.h; ++row) {
    memcpy(flippedBuffer.get() + (_viewPort.h - row - 1) * _viewPort.w * 4,
           buffer.get() + row * _viewPort.w * 4, _viewPort.w * 4);
  }
