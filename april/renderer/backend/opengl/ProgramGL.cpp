...
namespace {
std::string vsPreDefine(
    "#version 100\n precision highp float;\n precision highp int;\n");
std::string fsPreDefine("precision mediump float;\n precision mediump int;\n");
}  // namespace

ProgramGL::ProgramGL(const std::string& vertexShader,
                     const std::string& fragmentShader)
    : Program(vertexShader, fragmentShader) {
#if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID
  // some device required manually specify the precision qualifiers for vertex
  // shader.
  _vertexShaderModule =
      static_cast<ShaderModuleGL*>(ShaderCache::newVertexShaderModule(
          std::move(vsPreDefine + vertex_shader_)));
  _fragmentShaderModule =
      static_cast<ShaderModuleGL*>(ShaderCache::newFragmentShaderModule(
          std::move(fsPreDefine + fragment_shader_)));
#else
  _vertexShaderModule = static_cast<ShaderModuleGL*>(
      ShaderCache::newVertexShaderModule(_vertexShader));
  _fragmentShaderModule = static_cast<ShaderModuleGL*>(
      ShaderCache::newFragmentShaderModule(_fragmentShader));
#endif

  SAFE_RETAIN(_vertexShaderModule);
  SAFE_RETAIN(_fragmentShaderModule);
  compileProgram();
  computeUniformInfos();
  computeLocations();
  ...
  _backToForegroundListener = EventListenerCustom::create(
      ...
      [this](EventCustom*) { this->reloadProgram(); });
  Director::getInstance()
      ->getEventDispatcher()
      ->addEventListenerWithFixedPriority(_backToForegroundListener, -1);
#endif
}

ProgramGL::~ProgramGL() {
  SAFE_RELEASE(_vertexShaderModule);
  SAFE_RELEASE(_fragmentShaderModule);
...
void ProgramGL::reloadProgram() {
  ...
  static_cast<ShaderModuleGL*>(_vertexShaderModule)
      ->compileShader(backend::ShaderStage::VERTEX,
                      std::move(vsPreDefine + _vertexShader));
  static_cast<ShaderModuleGL*>(_fragmentShaderModule)
      ->compileShader(backend::ShaderStage::FRAGMENT,
                      std::move(fsPreDefine + _fragmentShader));
  compileProgram();
  computeUniformInfos();
...
void ProgramGL::computeLocations() {
  std::fill(_builtinAttributeLocation,
            _builtinAttributeLocation + ATTRIBUTE_MAX, -1);
  //    std::fill(_builtinUniformLocation, _builtinUniformLocation +
  //    UNIFORM_MAX, -1);
...
bool ProgramGL::getAttributeLocation(const std::string& attributeName,
                                     unsigned int& location) const {
  GLint loc = glGetAttribLocation(_program, attributeName.c_str());
  if (-1 == loc) {
    printf("april3d: %s: can not find vertex attribute of %s", __FUNCTION__,
           attributeName.c_str());
    return false;
  }
...
const std::unordered_map<std::string, AttributeBindInfo>
ProgramGL::getActiveAttributes() const {
  ...
  if (numOfActiveAttributes <= 0) return attributes;
  ...
  for (int i = 0; i < numOfActiveAttributes; i++) {
    glGetActiveAttrib(_program, i, MAX_ATTRIBUTE_NAME_LENGTH, &attrNameLen,
                      &attrSize, &attrType, attrName.data());
    CHECK_GL_ERROR_DEBUG();
    info.attributeName =
        std::string(attrName.data(), attrName.data() + attrNameLen);
...
#define MAX_UNIFORM_NAME_LENGTH 256
  ...
  for (int i = 0; i < numOfUniforms; ++i) {
    glGetActiveUniform(_program, i, MAX_UNIFORM_NAME_LENGTH, &length,
                       &uniform.count, &uniform.type, uniformName);
    ...
    _maxLocation = _maxLocation <= uniform.location ? (uniform.location + 1)
                                                    : _maxLocation;
  }
  free(uniformName);
}
...
UniformLocation ProgramGL::getUniformLocation(
  ...
  if (_activeUniformInfos.find(uniform) != _activeUniformInfos.end()) {
    ...
    uniformLocation.location[0] =
        _mapToOriginalLocation.at(uniformInfo.location);
#else
...
int ProgramGL::getMaxVertexLocation() const { return _maxLocation; }
int ProgramGL::getMaxFragmentLocation() const { return _maxLocation; }

#if CC_ENABLE_CACHE_TEXTURE_DATA
int ProgramGL::getMappedLocation(int location) const {
  if (_mapToCurrentActiveLocation.find(location) !=
      _mapToCurrentActiveLocation.end())
...
const UniformInfo& ProgramGL::getActiveUniformInfo(ShaderStage stage,
                                                   int location) const {
  return std::move(UniformInfo{});
}

const std::unordered_map<std::string, UniformInfo>&
ProgramGL::getAllActiveUniformInfo(ShaderStage stage) const {
