...
GLint UtilsGL::toGLMinFilter(SamplerFilter minFilter, bool hasMipmaps,
  ...
  if (hasMipmaps && !isPow2) {
    printf(
        "Change minification filter to either NEAREST or LINEAR since "
        "non-power-of-two texture occur in %s %s %d",
...
GLint UtilsGL::toGLAddressMode(SamplerAddressMode addressMode, bool isPow2) {
  ...
  if (!isPow2 && (addressMode != SamplerAddressMode::CLAMP_TO_EDGE)) {
    printf(
        "Change texture wrap mode to CLAMP_TO_EDGE since non-power-of-two "
        "texture occur in %s %s %d",
        __FILE__, __FUNCTION__, __LINE__);
...
void UtilsGL::toGLTypes(PixelFormat textureFormat, GLint &internalFormat,
