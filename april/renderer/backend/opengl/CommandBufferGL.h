 ...
 * A command buffer stores encoded commands until the buffer is committed for
...
class CommandBufferGL final : public CommandBuffer {
 public:
   ...
   * @param descriptor Specifies a group of render targets that hold the results
   * of a render pass.
   */
   ...
   * @param renderPipeline An object that contains the graphics functions and
   * configuration state used in a render pass.
   */
   ...
   */
  virtual void setViewport(int x, int y, unsigned int w,
   ...
   * @param programState A programState object that hold the uniform and texture
   * data.
   */
   ...
   * @ see `drawElements(PrimitiveType primitiveType, IndexFormat indexType,
   ...
   * @param primitiveType The type of primitives that elements are assembled
   * into.
   ...
   * @param primitiveType The type of primitives that elements are assembled
   * into.
   * @param indexType The type if indexes, either 16 bit integer or 32 bit
   * integer.
   * @param count The number of indexes to read from the index buffer for each
   * instance.
   * @param offset Byte offset within indexBuffer to start reading indexes from.
   * @see `setIndexBuffer(Buffer* buffer)`
   * @see `drawArrays(PrimitiveType primitiveType, unsigned int start,  unsigned
   * int count)`
   */
  virtual void drawElements(PrimitiveType primitiveType, IndexFormat indexType,
                            std::size_t count, std::size_t offset) override;
  ...
  /**
   * Present a drawable and commit a command buffer so it can be executed as
   * soon as possible.
   */
  ...
  virtual void setDepthStencilState(
      DepthStencilState* depthStencilState) override;
  ...
  virtual void captureScreen(
      std::function<void(const unsigned char*, int, int)> callback) override;
 ...
 private:
  ...
  void setUniform(bool isArray, GLuint location, unsigned int size,
                  GLenum uniformType, void* data) const;
  void cleanResources();
  void applyRenderPassDescriptor(const RenderPassDescriptor& descirptor);

  // The frame buffer generated by engine. All frame buffer other than default
  // frame buffer share it.
  ...
  GLint _defaultFBO =
      0;  // The value gets from glGetIntegerv, so need to use GLint
