...
class UtilsGL {
 public:
  /**
   * Convert attribute enum classs type to GLenum type. i.e. convert
   * VertexFormat::FLOAT4 to GL_FLOAT.
  ...
  /**
   * Convert attribute component count to GLsizei. i.e. convert
   * VertexFormat::FLOAT4 to 4.
  ...
  /**
   * Get opengl data type size in bytes. i.e. GL_FLOAT_VEC4 data type size is
   * sizeof(GLfloat) * 4.
  ...
  /**
   * Convert magnification filter to GLint. i.e. convert SamplerFilter::LINEAR
   * to GL_LINEAR.
  ...
  /**
   * Convert minifying filter to GLint. i.e. convert SamplerFilter::LINEAR to
   * GL_LINEAR. If mipmaps is enabled and texture width and height are not power
   * of two, then if minFilter is SamplerFilter::LINEAR, GL_LINEAR is returned,
   * otherwise return GL_NEAREST.
  ...
  /**
   * Convert wrap parameter for texture coordinate to GLint. i.e. convert
   * SamplerAddressMode::REPEAT to GL_REPEAT. If texture width and height are
   * not power of 2, then GL_CLAMP_TO_EDGE is returned.
   ...
   */
  static void toGLTypes(PixelFormat textureFormat, GLint &internalFormat,
                        GLuint &format, GLenum &type, bool &isCompressed);

  /**
   * Convert compare function to GLenum. i.e. convert CompareFunction::NEVER to
   * GL_NEVER.
  ...
  /**
   * Convert stencil operation to GLenum. i.e. convert StencilOperation::KEEP to
   * GL_KEEP.
  ...
  /**
   * Convert blend operation to GLenum. i.e. convert BlendOperation::ADD to
   * GL_FUNC_ADD.
  ...
  /**
   * Convert primitive type to GLenum. i.e. convert PrimitiveType::TRIANGLE to
   * GL_TRIANGLES.
  ...
  /**
   * Convert index type to GLenum. i.e. convert IndexFormat::U_INT to
   * GL_UNSIGNED_INT.
