...
void TextureInfoGL::applySamplerDescriptor(const SamplerDescriptor& descriptor,
                                           bool isPow2, bool hasMipmaps) {
  ...
  if (descriptor.minFilter != SamplerFilter::DONT_CARE) {
    minFilterGL =
        UtilsGL::toGLMinFilter(descriptor.minFilter, hasMipmaps, isPow2);
  }
...
Texture2DGL::Texture2DGL(const TextureDescriptor& descriptor)
  ...
  // Listen this event to restored texture id after coming to foreground on
  // Android.
...
void Texture2DGL::updateTextureDescriptor(
    const april::backend::TextureDescriptor& descriptor) {
  TextureBackend::updateTextureDescriptor(descriptor);
  UtilsGL::toGLTypes(descriptor.textureFormat, _textureInfo.internalFormat,
                     _textureInfo.format, _textureInfo.type, _isCompressed);

  bool isPow2 = ISPOW2(_width) && ISPOW2(_height);
  _textureInfo.magFilterGL =
      UtilsGL::toGLMagFilter(descriptor.samplerDescriptor.magFilter);
  _textureInfo.minFilterGL = UtilsGL::toGLMinFilter(
      descriptor.samplerDescriptor.minFilter, _hasMipmaps, isPow2);

  _textureInfo.sAddressModeGL = UtilsGL::toGLAddressMode(
      descriptor.samplerDescriptor.sAddressMode, isPow2);
  _textureInfo.tAddressModeGL = UtilsGL::toGLAddressMode(
      descriptor.samplerDescriptor.tAddressMode, isPow2);
...
Texture2DGL::~Texture2DGL() {
  if (_textureInfo.texture) glDeleteTextures(1, &_textureInfo.texture);
...
void Texture2DGL::updateData(uint8_t* data, std::size_t width,
                             std::size_t height, std::size_t level) {
  // Set the row align only when mipmapsNum == 1 and the data is uncompressed
  auto mipmapEnalbed = isMipmapEnabled(_textureInfo.minFilterGL) ||
                       isMipmapEnabled(_textureInfo.magFilterGL);
  if (!mipmapEnalbed) {
  ...
  glTexImage2D(GL_TEXTURE_2D, level, _textureInfo.internalFormat, width, height,
               0, _textureInfo.format, _textureInfo.type, data);
  CHECK_GL_ERROR_DEBUG();

  if (!_hasMipmaps && level > 0) _hasMipmaps = true;
}

void Texture2DGL::updateCompressedData(uint8_t* data, std::size_t width,
  ...
  glCompressedTexImage2D(GL_TEXTURE_2D, level, _textureInfo.internalFormat,
...
void Texture2DGL::updateSubData(std::size_t xoffset, std::size_t yoffset,
...
void Texture2DGL::updateCompressedSubData(std::size_t xoffset,
  ...
  glCompressedTexSubImage2D(GL_TEXTURE_2D, level, xoffset, yoffset, width,
                            height, _textureInfo.format, dataLen, data);
  CHECK_GL_ERROR_DEBUG();
...
void Texture2DGL::generateMipmaps() {
  if (TextureUsage::RENDER_TARGET == _textureUsage) return;
  ...
  if (flipImage) {
    ...
    for (int i = 0; i < height; ++i) {
      memcpy(&flippedImage[i * bytePerRow],
             &image[(height - i - 1) * bytePerRow], bytePerRow);
    }
...
TextureCubeGL::TextureCubeGL(const TextureDescriptor& descriptor)
  ...
  UtilsGL::toGLTypes(_textureFormat, _textureInfo.internalFormat,
                     _textureInfo.format, _textureInfo.type, _isCompressed);
  glGenTextures(1, &_textureInfo.texture);
  updateSamplerDescriptor(descriptor.samplerDescriptor);
  ...
  // Android.
  _backToForegroundListener = EventListenerCustom::create(
      EVENT_COME_TO_FOREGROUND, [this](EventCustom*) {
        glGenTextures(1, &(this->_textureInfo.texture));
...
void TextureCubeGL::getBytes(
  ...
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                         GL_TEXTURE_CUBE_MAP, _textureInfo.texture, 0);
  glPixelStorei(GL_PACK_ALIGNMENT, 1);
