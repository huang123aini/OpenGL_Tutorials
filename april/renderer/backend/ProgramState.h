...
struct TextureInfo {
  TextureInfo(const std::vector<uint32_t>& _slots,
 ...
 */
class ProgramState : public april::Ref {
 public:
  using UniformCallback =
      std::function<void(ProgramState*, const UniformLocation&)>;
  ...
  /**
   * Specify the value of a uniform variable for the current program state
   * object.
   * @param uniformLocation Specifies the uniform location.
   * @param data Specifies the new values to be used for the specified uniform
   * variable.
   ...
   * @see `backend::UniformLocation getUniformLocation(backend::Uniform name)
   * const`
   */
  backend::UniformLocation getUniformLocation(const std::string& uniform) const;

  /**
   * Get uniform location in a more efficient way by the given built-in uniform
   * name.
   ...
   * @return Vertex texture informations. Key is the texture location, Value
   * store the texture informations
   */
  inline const std::unordered_map<int, TextureInfo>& getVertexTextureInfos()
      const {
   ...
   * @return Fragment texture informations. Key is the texture location, Value
   * store the texture informations
   */
  inline const std::unordered_map<int, TextureInfo>& getFragmentTextureInfos()
      const {
   ...
   */
  inline const std::unordered_map<UniformLocation, UniformCallback,
                                  UniformLocation>&
   ...
   * @param[out] buffer Specifies the pointer points to a vertex uniform
   * storage.
  ...
  /**
   * Get fragment uniform buffer. The buffer store all the fragment uniform's
   * data for metal.
   * @param[out] buffer Specifies the pointer points to a fragment uniform
   * storage.
   * @param[out] size Specifies the size of the buffer in bytes.
   */
  ...
  /**
   * An abstract base class that can be extended to support custom material auto
   * bindings.
   *
   * Implementing a custom auto binding resolver allows the set of built-in
   * parameter auto bindings to be extended or overridden. Any parameter auto
   * binding that is set on a material will be forwarded to any custom auto
   * binding resolvers, in the order in which they are registered. If a
   * registered resolver returns true (specifying that it handles the specified
   * autoBinding), no further code will be executed for that autoBinding. This
   * allows auto binding resolvers to not only implement new/custom binding
   * strings, but it also lets them override existing/built-in ones. For this
   * reason, you should ensure that you ONLY return true if you explicitly
   * handle a custom auto binding; return false otherwise.
   *
   * Note that the custom resolver is called only once for a GLProgramState
   * object when its node binding is initially set. This occurs when a material
   * is initially bound to a Node. The resolver is NOT called each frame or each
   * time the GLProgramState is bound.
   *
   * If no registered resolvers explicitly handle an auto binding, the binding
   * will attempt to be resolved using the internal/built-in resolver, which is
   * able to handle any auto bindings found in the GLProgramState::AutoBinding
   * enumeration.
   *
   * When an instance of a class that extends AutoBindingResolver is created, it
   * is automatically registered as a custom auto binding handler. Likewise, it
   * is automatically unregistered on destruction.
   *
   * @script{ignore}
   */
  class AutoBindingResolver {
   public:
     ...
     *
     * Implementations of this method should do a string comparison on the
     * passed in name parameter and decide whether or not they should handle the
     ...
     * Otherwise, the parameter should be set or bound and true should be
     * returned.
     *
     ...
     *
     * @return True if the auto binding is handled and the associated parameter
   ...
   *
   * This method parses the passed in autoBinding string and attempts to convert
   * it to an enumeration value. If it matches to one of the predefined strings,
   * it will create a callback to get the correct value at runtime.
   *
   * @param uniformName The name of the material parameter to store an
   * auto-binding for.
   * @param autoBinding A string matching one of the built-in AutoBinding enum
   * constants.
   */
 ...
 protected:
   ...
   * @param size Specifies the uniform data size.
   */
  void setVertexUniform(int location, const void* data, std::size_t size,
                        std::size_t offset);
  ...
  std::unordered_map<UniformLocation, UniformCallback, UniformLocation>
      _callbackUniforms;
  ...
  std::shared_ptr<VertexLayout> _vertexLayout =
      std::make_shared<VertexLayout>();
};
