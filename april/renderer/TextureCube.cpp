...
Image* createImage(const std::string& path) {
  ...
  // all images are handled by UIImage except PVR extension that is handled by
  // our own handler
  Image* image = nullptr;
  do {
...
TextureCube::~TextureCube() { CC_SAFE_RELEASE_NULL(_texture); }
...
TextureCube* TextureCube::create(const std::string& positive_x,
  ...
  if (ret && ret->init(positive_x, negative_x, positive_y, negative_y,
...
bool TextureCube::init(const std::string& positive_x,
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (imageSize != img->getWidth()) {
      CCASSERT(
          ...
          "TextureCubmap: texture of each face should have same dimension");
      return false;
    }
  }
  ...
  textureDescriptor.samplerDescriptor.magFilter =
      backend::SamplerFilter::LINEAR;
  textureDescriptor.samplerDescriptor.sAddressMode =
      backend::SamplerAddressMode::CLAMP_TO_EDGE;
  textureDescriptor.samplerDescriptor.tAddressMode =
      backend::SamplerAddressMode::CLAMP_TO_EDGE;
  _texture = static_cast<backend::TextureCubemapBackend*>(
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (ePixelFmt != backend::PixelFormat::RGBA8888 &&
        ePixelFmt != backend::PixelFormat::DEFAULT) {
      size_t len = 0;
      backend::PixelFormatUtils::convertDataToFormat(
          pData, img->getDataLen(), ePixelFmt, backend::PixelFormat::RGBA8888,
          &cData, &len);
      if (cData != pData)  // convert error
      {
        ...
        CCASSERT(false,
                 "error: CubeMap texture may be incorrect, failed to convert "
                 "pixel format data to RGBA8888");
      }
    }

    _texture->updateFaceData(static_cast<backend::TextureCubeFace>(i), useData);

    if (cData != pData) free(cData);

    if (pData != img->getData()) free(pData);
  }
...
bool TextureCube::reloadTexture() {
  return init(_imgPath[0], _imgPath[1], _imgPath[2], _imgPath[3], _imgPath[4],
              _imgPath[5]);
}
