...
void convertAI88ToBGR565(const unsigned char* data, size_t dataLen,
                         unsigned char* outData) {
...
void convertAI88ToBGR5A1(const unsigned char* data, size_t dataLen,
                         unsigned char* outData) {
  uint16_t* out16 = (uint16_t*)outData;
  for (ssize_t i = 0, l = dataLen - 1; i < l; i += 2) {
...
void convertAI88ToABGR4(const unsigned char* data, size_t dataLen,
                        unsigned char* outData) {
  uint16_t* out16 = (uint16_t*)outData;
  for (ssize_t i = 0, l = dataLen - 1; i < l; i += 2) {
    *out16++ = (data[i] & 0x00F0) << 8         // R
...
void convertI8ToBGR5A1(const unsigned char* data, size_t dataLen,
                       unsigned char* outData) {
  uint16_t* out16 = (uint16_t*)outData;
  for (size_t i = 0; i < dataLen; ++i) {
...
void convertI8ToBGR565(const unsigned char* data, size_t dataLen,
                       unsigned char* outData) {
  uint16_t* out16 = (uint16_t*)outData;
  for (size_t i = 0; i < dataLen; ++i) {
...
void convertI8ToABGR4(const unsigned char* data, size_t dataLen,
                      unsigned char* outData) {
  uint16_t* out16 = (uint16_t*)outData;
  for (size_t i = 0; i < dataLen; ++i) {
...
void convertRGB888ToB5G6R5(const unsigned char* data, size_t dataLen,
                           unsigned char* out) {
  uint16_t* outData = (uint16_t*)out;
  for (size_t i = 0; i < dataLen; i += 3) {
    *outData++ = ((data[i] & 0xF8) << 8) | ((data[i + 1] & 0xFC) << 3) |
                 ((data[i + 2] & 0xF8) >> 3);
  }
}

// RRRRRRRRGGGGGGGGBBBBBBBB -> BBBBBGGG GGRRRRRA
void convertRGB888ToBGR5A1(const unsigned char* data, size_t dataLen,
                           unsigned char* out) {
  uint16_t* outData = (uint16_t*)out;
  for (size_t i = 0; i < dataLen; i += 3) {
    *outData++ = ((data[i] & 0xF8) << 7) | ((data[i + 1] & 0xF8) << 2) |
                 ((data[i + 2] & 0xF8) >> 3) | 0x8000;
  }
}

// RRRRRRRRGGGGGGGGBBBBBBBB -> AAAABBBB GGGGRRRR
void convertRGB888ToABGR4(const unsigned char* data, size_t dataLen,
                          unsigned char* out) {
  uint16_t* outData = (uint16_t*)out;
  for (size_t i = 0; i < dataLen; i += 3) {
...
void convertRGBA8888ToBGR565(const unsigned char* data, size_t dataLen,
                             unsigned char* out) {
  uint16_t* outData = (uint16_t*)out;
...
void convertRGBA8888ToABGR4(const unsigned char* data, size_t dataLen,
                            unsigned char* out) {
  uint16_t* outData = (uint16_t*)out;
...
void convertRGBA8888ToBGR5A1(const unsigned char* data, size_t dataLen,
                             unsigned char* out) {
  uint16_t* outData = (uint16_t*)out;
  for (size_t i = 0; i < dataLen; i += 4) {
...
void convertRGB5A1ToBGR5A1(const unsigned char* data, size_t dataLen,
                           unsigned char* out) {
  const size_t pixelLen = dataLen / 2;
...
// void convertRGB565ToB5G6R5(const unsigned char *data, ssize_t dataLen,
// unsigned char *out)
...
// void convertRGBA4444ToABGR4444(const unsigned char *data, ssize_t dataLen,
// unsigned char *out)
...
april::backend::PixelFormat convertI8ToFormat(const unsigned char* data,
                                              size_t dataLen,
                                              PixelFormat format,
                                              unsigned char** outData,
                                              size_t* outDataLen) {
  switch (format) {
    ...
    default:
      ...
      if (format != PixelFormat::AUTO && format != PixelFormat::I8) {
        printf(
            "Can not convert image format PixelFormat::I8 to format ID:%d, we "
            "will use it's origin format PixelFormat::I8",
...
april::backend::PixelFormat convertAI88ToFormat(const unsigned char* data,
                                                size_t dataLen,
                                                PixelFormat format,
                                                unsigned char** outData,
                                                size_t* outDataLen) {
  switch (format) {
    ...
    default:
      ...
      if (format != PixelFormat::AUTO && format != PixelFormat::AI88) {
        printf(
            "Can not convert image format PixelFormat::AI88 to format ID:%d, "
            "we will use it's origin format PixelFormat::AI88",
            static_cast<int>(format));
      }
...
april::backend::PixelFormat convertRGB888ToFormat(const unsigned char* data,
                                                  size_t dataLen,
                                                  PixelFormat format,
                                                  unsigned char** outData,
                                                  size_t* outDataLen) {
  switch (format) {
    ...
    default:
      ...
      if (format != PixelFormat::AUTO && format != PixelFormat::RGB888) {
        printf(
            "Can not convert image format PixelFormat::RGB888 to format ID:%d, "
            "we will use it's origin format PixelFormat::RGB888",
            static_cast<int>(format));
      }
...
april::backend::PixelFormat convertRGBA8888ToFormat(const unsigned char* data,
                                                    size_t dataLen,
                                                    PixelFormat format,
                                                    unsigned char** outData,
                                                    size_t* outDataLen) {
  switch (format) {
    ...
    default:
      ...
      if (format != PixelFormat::AUTO && format != PixelFormat::RGBA8888) {
        printf(
            "Can not convert image format PixelFormat::RGBA8888 to format "
            "ID:%d, we will use it's origin format PixelFormat::RGBA8888",
            static_cast<int>(format));
      }
...
april::backend::PixelFormat convertRGB5A1ToFormat(const unsigned char* data,
                                                  size_t dataLen,
                                                  PixelFormat format,
                                                  unsigned char** outData,
                                                  size_t* outDataLen) {
  switch (format) {
    ...
    default:
      ...
      if (format != PixelFormat::AUTO && format != PixelFormat::RGBA8888) {
        printf(
            "Can not convert image format PixelFormat::RGB5A1 to format ID:%d, "
            "we will use it's origin format PixelFormat::RGB51A",
            static_cast<int>(format));
      }
      *outData = (unsigned char*)data;
      *outDataLen = dataLen;
...
april::backend::PixelFormat convertRGB565ToFormat(const unsigned char* data,
                                                  size_t dataLen,
                                                  PixelFormat format,
                                                  unsigned char** outData,
                                                  size_t* outDataLen) {
  switch (format) {
    ...
    default:
      ...
      if (format != PixelFormat::AUTO && format != PixelFormat::RGBA8888) {
        printf(
            "Can not convert image format PixelFormat::RGB565 to format ID:%d, "
            "we will use it's origin format PixelFormat::RGB565",
            static_cast<int>(format));
      }
      *outData = (unsigned char*)data;
      *outDataLen = dataLen;
...
april::backend::PixelFormat convertA8ToFormat(const unsigned char* data,
                                              size_t dataLen,
                                              PixelFormat format,
                                              unsigned char** outData,
                                              size_t* outDataLen) {
  switch (format) {
    ...
    default:
      ...
      if (format != PixelFormat::AUTO && format != PixelFormat::RGBA8888) {
        printf(
            "Can not convert image format PixelFormat::A8 to format ID:%d, we "
            "will use it's origin format PixelFormat::A8",
            static_cast<int>(format));
      }
      *outData = (unsigned char*)data;
      *outDataLen = dataLen;
...
april::backend::PixelFormat convertRGBA4444ToFormat(const unsigned char* data,
                                                    size_t dataLen,
                                                    PixelFormat format,
                                                    unsigned char** outData,
                                                    size_t* outDataLen) {
  switch (format) {
    ...
    default:
      ...
      if (format != PixelFormat::AUTO && format != PixelFormat::RGBA8888) {
        printf(
            "Can not convert image format PixelFormat::RGBA444 to format "
            "ID:%d, we will use it's origin format PixelFormat::RGBA4444",
            static_cast<int>(format));
      }
      *outData = (unsigned char*)data;
      *outDataLen = dataLen;
 ...
 */
april::backend::PixelFormat convertDataToFormat(
    const unsigned char* data, size_t dataLen, PixelFormat originFormat,
  ...
  switch (originFormat) {
    ...
    case PixelFormat::RGBA8888:
      return convertRGBA8888ToFormat(data, dataLen, format, outData,
                                     outDataLen);
    case PixelFormat::RGB5A1:
...
#ifdef CC_USE_METAL
    case PixelFormat::RGBA4444:
      return convertRGBA4444ToFormat(data, dataLen, format, outData,
                                     outDataLen);
    case PixelFormat::A8:
    ...
    case PixelFormat::BGRA8888:
      return convertBGRA8888ToFormat(data, dataLen, format, outData,
                                     outDataLen);
    default:
      printf("unsupported conversion from format %d to format %d",
             static_cast<int>(originFormat), static_cast<int>(format));
      *outData = (unsigned char*)data;
      *outDataLen = dataLen;
      return originFormat;
  }
