...
void RenderQueue::sort() {
  ...
  std::stable_sort(std::begin(_commands[QUEUE_GROUP::TRANSPARENT_3D]),
                   std::end(_commands[QUEUE_GROUP::TRANSPARENT_3D]),
                   compare3DCommand);
  ...
  std::stable_sort(std::begin(_commands[QUEUE_GROUP::GLOBALZ_POS]),
                   std::end(_commands[QUEUE_GROUP::GLOBALZ_POS]),
                   compareRenderCommand);
}
...
Renderer::Renderer() {
  ...
  _triBatchesToDraw = (TriBatchToDraw*)malloc(sizeof(_triBatchesToDraw[0]) *
...
void Renderer::addCommand(RenderCommand* command, int renderQueueID) {
  ...
  CCASSERT(command->getType() != RenderCommand::Type::UNKNOWN_COMMAND,
    ...
    case RenderCommand::Type::TRIANGLES_COMMAND: {
      ...
      if (_queuedTotalVertexCount + cmd->getVertexCount() > VBO_SIZE ||
        ...
        CCASSERT(cmd->getVertexCount() >= 0 && cmd->getVertexCount() < VBO_SIZE,
                 "VBO for vertex is not big enough, please break the data down "
                 "or use customized render command");
        CCASSERT(
            cmd->getIndexCount() >= 0 && cmd->getIndexCount() < INDEX_VBO_SIZE,
            "VBO for index is not big enough, please break the data down or "
            "use customized render command");
...
void Renderer::captureScreen(RenderCommand* command) {
  _commandBuffer->captureScreen(
      static_cast<CaptureScreenCallbackCommand*>(command)->func);
}
...
void Renderer::visitRenderQueue(RenderQueue& queue) {
  ...
  setDepthWrite(false);
  doVisitRenderQueue(
      queue.getSubQueue(RenderQueue::QUEUE_GROUP::TRANSPARENT_3D));
  popStateBlock();
...
void Renderer::doVisitRenderQueue(
...
void Renderer::beginFrame() { _commandBuffer->beginFrame(); }
...
void Renderer::setStencilCompareFunction(backend::CompareFunction func,
                                         unsigned int ref,
...
void Renderer::setStencilOperation(
    backend::StencilOperation stencilFailureOp,
    backend::StencilOperation depthFailureOp,
    backend::StencilOperation stencilDepthPassOp) {
  _depthStencilDescriptor.frontFaceStencil.stencilFailureOperation =
      stencilFailureOp;
  _depthStencilDescriptor.backFaceStencil.stencilFailureOperation =
      stencilFailureOp;

  _depthStencilDescriptor.frontFaceStencil.depthFailureOperation =
      depthFailureOp;
  _depthStencilDescriptor.backFaceStencil.depthFailureOperation =
      depthFailureOp;

  _depthStencilDescriptor.frontFaceStencil.depthStencilPassOperation =
      stencilDepthPassOp;
  _depthStencilDescriptor.backFaceStencil.depthStencilPassOperation =
      stencilDepthPassOp;
}
...
backend::StencilOperation Renderer::getStencilPassDepthFailureOperation()
    const {
...
unsigned int Renderer::getStencilReferenceValue() const { return _stencilRef; }
...
void Renderer::fillVerticesAndIndices(const TrianglesCommand* cmd,
  ...
  memcpy(&_verts[_filledVertex], cmd->getVertices(),
         sizeof(V3F_C4B_T2F) * vertexCount);
  ...
  for (size_t i = 0; i < indexCount; ++i) {
    _indices[_filledIndex + i] =
        vertexBufferOffset + _filledVertex + indices[i];
  }
...
void Renderer::drawBatchedTriangles() {
  if (_queuedTriangleCommands.empty()) return;

    /************** 1: Setup up vertices/indices *************/
#ifdef CC_USE_METAL
  unsigned int vertexBufferFillOffset =
      _queuedTotalVertexCount - _queuedVertexCount;
  unsigned int indexBufferFillOffset =
      _queuedTotalIndexCount - _queuedIndexCount;
#else
    ...
    if (batchable && (prevMaterialID == currentMaterialID || firstCommand)) {
      ...
      if (!firstCommand) {
        ...
        _triBatchesToDraw[batchesTotal].offset =
            _triBatchesToDraw[batchesTotal - 1].offset +
      ...
      if (!batchable) currentMaterialID = -1;
    }

    // capacity full ?
    if (batchesTotal + 1 >= _triBatchesToDrawCapacity) {
      _triBatchesToDrawCapacity *= 1.4;
      _triBatchesToDraw = (TriBatchToDraw*)realloc(
          _triBatchesToDraw,
  ...
  for (int i = 0; i < batchesTotal; ++i) {
    ...
    auto& pipelineDescriptor =
        _triBatchesToDraw[i].cmd->getPipelineDescriptor();
    _commandBuffer->setProgramState(pipelineDescriptor.programState);
    _commandBuffer->drawElements(
        backend::PrimitiveType::TRIANGLE, backend::IndexFormat::U_SHORT,
...
void Renderer::drawCustomCommand(RenderCommand* command) {
  ...
  if (CustomCommand::DrawType::ELEMENT == drawType) {
    ...
    _commandBuffer->drawElements(cmd->getPrimitiveType(), cmd->getIndexFormat(),
...
void Renderer::flush2D() { flushTriangles(); }

void Renderer::flush3D() {
  // TODO 3d batch rendering
}

void Renderer::flushTriangles() { drawBatchedTriangles(); }
...
bool Renderer::checkVisibility(const Mat4& transform, const Size& size) {
  ...
  //  only cull the default camera. The culling algorithm is valid for default
  //  camera.
  ...
  float wshw =
      std::max(fabsf(hSizeX * transform.m[0] + hSizeY * transform.m[4]),
               fabsf(hSizeX * transform.m[0] - hSizeY * transform.m[4]));
  float wshh =
      std::max(fabsf(hSizeX * transform.m[1] + hSizeY * transform.m[5]),
...
void Renderer::setRenderPipeline(
  ...
  auto needDepthStencilAttachment = renderPassDescriptor.depthTestEnabled ||
                                    renderPassDescriptor.stencilTestEnabled;
  if (needDepthStencilAttachment) {
    depthStencilState =
        device->createDepthStencilState(_depthStencilDescriptor);
  }
...
void Renderer::beginRenderPass(RenderCommand* cmd) {
  ...
  _commandBuffer->setViewport(_viewport.x, _viewport.y, _viewport.w,
                              _viewport.h);
  _commandBuffer->setCullMode(_cullMode);
  _commandBuffer->setWinding(_winding);
  _commandBuffer->setScissorRect(_scissorState.isEnabled, _scissorState.rect.x,
                                 _scissorState.rect.y, _scissorState.rect.width,
                                 _scissorState.rect.height);
  setRenderPipeline(cmd->getPipelineDescriptor(), _renderPassDescriptor);

  _commandBuffer->setStencilReferenceValue(_stencilRef);
}

void Renderer::setRenderTarget(RenderTargetFlag flags,
  ...
  if (flags & RenderTargetFlag::COLOR) {
    ...
    if (colorAttachment)
      _renderPassDescriptor.colorAttachmentsTexture[0] =
          colorAttachment->getBackendTexture();
    else
  ...
  if (flags & RenderTargetFlag::DEPTH) {
    ...
    if (depthAttachment)
      _renderPassDescriptor.depthAttachmentTexture =
          depthAttachment->getBackendTexture();
    else
  ...
  if (flags & RenderTargetFlag::STENCIL) {
    ...
    if (_stencilAttachment)
      _renderPassDescriptor.stencilAttachmentTexture =
          stencilAttachment->getBackendTexture();
    else
...
void Renderer::clear(ClearFlag flags, const Color4F& color, float depth,
                     unsigned int stencil, float globalOrder) {
  _clearFlag = flags;
  ...
  command->func = [=]() -> void {
    ...
    if (flags & ClearFlag::COLOR) {
      ...
      descriptor.colorAttachmentsTexture[0] =
          _renderPassDescriptor.colorAttachmentsTexture[0];
    }
    if (flags & ClearFlag::DEPTH) {
      ...
      descriptor.depthAttachmentTexture =
          _renderPassDescriptor.depthAttachmentTexture;
    }
    if (flags & ClearFlag::STENCIL) {
      ...
      descriptor.stencilAttachmentTexture =
          _renderPassDescriptor.stencilAttachmentTexture;
    }
...
Texture2D* Renderer::getColorAttachment() const { return _colorAttachment; }

Texture2D* Renderer::getDepthAttachment() const { return _depthAttachment; }

Texture2D* Renderer::getStencilAttachment() const { return _stencilAttachment; }

const Color4F& Renderer::getClearColor() const { return _clearColor; }
...
ClearFlag Renderer::getClearFlag() const { return _clearFlag; }
...
bool Renderer::getScissorTest() const { return _scissorState.isEnabled; }
...
Renderer::TriangleCommandBufferManager::~TriangleCommandBufferManager() {
  for (auto& vertexBuffer : _vertexBufferPool) vertexBuffer->release();

  for (auto& indexBuffer : _indexBufferPool) indexBuffer->release();
}

void Renderer::TriangleCommandBufferManager::init() { createBuffer(); }
...
backend::Buffer* Renderer::TriangleCommandBufferManager::getVertexBuffer()
    const {
  return _vertexBufferPool[_currentBufferIndex];
}

backend::Buffer* Renderer::TriangleCommandBufferManager::getIndexBuffer()
    const {
...
void Renderer::TriangleCommandBufferManager::createBuffer() {
  ...
  auto vertexBuffer = device->newBuffer(Renderer::VBO_SIZE * sizeof(_verts[0]),
                                        backend::BufferType::VERTEX,
                                        backend::BufferUsage::DYNAMIC);
  if (!vertexBuffer) return;

  auto indexBuffer = device->newBuffer(
      Renderer::INDEX_VBO_SIZE * sizeof(_indices[0]),
  ...
  auto vertexBuffer = device->newBuffer(
      Renderer::VBO_SIZE * sizeof(V3F_C4B_T2F), backend::BufferType::VERTEX,
      backend::BufferUsage::DYNAMIC);
  if (!vertexBuffer) {
    free(tmpData);
    return;
  ...
  auto indexBuffer = device->newBuffer(
      Renderer::INDEX_VBO_SIZE * sizeof(unsigned short),
      backend::BufferType::INDEX, backend::BufferUsage::DYNAMIC);
  if (!indexBuffer) {
    free(tmpData);
    vertexBuffer->release();
  ...
  indexBuffer->updateData(tmpData,
                          Renderer::INDEX_VBO_SIZE * sizeof(unsigned short));
